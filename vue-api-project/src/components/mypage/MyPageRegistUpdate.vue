...
onMounted(() => {
  ...
  // 선호 스포츠 값 데이터 바인딩
  switch (mypageUser.fav_sport) {
    case "축구":
      fav_sport.value = "soccer";
      break;
    case "야구":
      fav_sport.value = "baseball";
      break;
    case "농구":
      fav_sport.value = "basketball";
      break;
    case "배구":
      fav_sport.value = "volleyball";
      break;
    // 필요에 따라 다른 스포츠도 추가
  }
});
...
const updateMypage = () => {
  ...
  const updatedInfo = {
    ...
    profile_image: selectedImage.value ? selectedImage.value.name : null,
  };
